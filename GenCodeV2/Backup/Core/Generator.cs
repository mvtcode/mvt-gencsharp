...
namespace CoreGen
{
    public static class Generator
    {
        ...
        public static void AddReaderSql(string dataType, string colName)
        {
            ...
            {
                case "int":
                    fieldText = String.Format("info.{0} = Int32.Parse(r[\"{0}\"].ToString());", propertyName) + "\n\t\t\t<column_name>";
                    break;
                case "bigint":
                    fieldText = String.Format("info.{0} = Int64.Parse(r[\"{0}\"].ToString());", propertyName) + "\n\t\t\t<column_name>";
                    break;
                case "nvarchar":
                case "varchar":
                case "text":
                case "ntext":
                    fieldText = String.Format("info.{0} = r[\"{0}\"].ToString();", propertyName) + "\n\t\t\t<column_name>";
                    break;
                case "datetime":
                case "smalldatetime":
                    fieldText = String.Format("info.{0} = Convert.ToDateTime(r[\"{0}\"]);", propertyName) + "\n\t\t\t<column_name>";
                    break;
                case "money":
                case "bigmoney":
                case "smallmoney":
                    fieldText = String.Format("info.{0} = double.Parse(r[\"{0}\"].ToString());", propertyName) + "\n\t\t\t<column_name>";
                    break;
                case "bit":
                    fieldText = String.Format("info.{0} = Convert.ToBoolean(r[\"{0}\"]);", propertyName) + "\n\t\t\t<column_name>";
                    break;
                case "tinyint":
                    fieldText = String.Format("info.{0} = Convert.ToByte(r[\"{0}\"].ToString());", propertyName) + "\n\t\t\t<column_name>";
                    break;
                case "decimal":
                    fieldText = String.Format("info.{0} = Convert.ToDecimal(r[\"{0}\"].ToString());", propertyName) + "\n\t\t\t<column_name>";
                    break;
                default:
                    fieldText = String.Format("info.{0} = (object)r[\"{0}\"].ToString();", propertyName) + "\n\t\t\t<column_name>";
                    break;
            }
